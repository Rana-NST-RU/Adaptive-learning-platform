...
import type { Question, AttemptResult, QuestionDifficulty } from '@/lib/api-client';
...
export default function PracticePage() {
  ...
  // Load concepts for picker
  useEffect(() => {
    graphApi.getGraph(selectedDomain).then((res) => {
  ...
  const startSession = useCallback(async () => {
    ...
    setPhase('loading');
    try {
      // Try fetching existing questions first
      ...
      if (status === 401) {
        alert('Your session has expired. Please log in again to practice.');
  ...
  const submitAnswer = useCallback(async () => {
    ...
    try {
      const res = await questionsApi.submitAttempt({
        questionId: q.id,
        answer,
        timeTakenMs,
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'transparent', padding: '24px' }}>
      ...
      {phase === 'loading' && <LoadingScreen concept={selectedConcept?.name ?? ''} />}
...
function ConceptPicker({
  concepts, selectedConcept, selectedDomain, selectedDiff, questionCount,
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        {concepts.map((c: ConceptOption) => {
          ...
          return (
            ...
            >
              <div style={{ fontWeight: 600, color: isSelected ? '#a5b4fc' : '#e2e8f0', fontSize: 14, marginBottom: 4 }}>{c.name}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ fontSize: 11, color: '#64748b', background: 'rgba(255,255,255,0.05)', padding: '2px 8px', borderRadius: 6 }}>{c.category}</span>
                <span style={{ fontSize: 11, color: '#f59e0b' }}>{'★'.repeat(c.difficulty)}</span>
              </div>
            </button>
          );
        })}
      ...
      <button
        ...
        style={{
          width: '100%', padding: '16px', borderRadius: 14, border: 'none', cursor: selectedConcept ? 'pointer' : 'not-allowed',
...
function LoadingScreen({ concept }: { concept: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: 24 }}>
      <div style={{ fontSize: 48, animation: 'spin 1s linear infinite' }}>⚙️</div>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 20, fontWeight: 700, color: '#f1f5f9', marginBottom: 8 }}>Generating AI questions…</div>
        <div style={{ color: '#94a3b8', fontSize: 15 }}>Preparing personalized questions on <strong style={{ color: '#a5b4fc' }}>{concept}</strong></div>
      </div>
    ...
    <div style={{ maxWidth: 760, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        {q.codeSnippet && (
          <pre style={{
            ...
            color: '#e2e8f0', marginBottom: 24, fontFamily: 'monospace',
          }}>
            <code>{q.codeSnippet}</code>
          </pre>
        )}

        <p style={{ fontSize: 19, fontWeight: 600, color: '#f1f5f9', lineHeight: 1.5, marginBottom: 28, margin: '0 0 28px' }}>
          {q.content}
        </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {q.options.map((opt: string) => {
              ...
              return (
                <button
                  key={opt}
                  onClick={() => !answered && onSelectOption(letter)}
                  disabled={answered}
                  style={{
                    padding: '14px 18px', borderRadius: 12, border: 'none', cursor: answered ? 'default' : 'pointer',
                    textAlign: 'left', fontWeight: 500, fontSize: 15, transition: 'all 0.2s',
          ...
          <div style={{ display: 'flex', gap: 16 }}>
            {['true', 'false'].map(val => {
              ...
              return (
                <button
                  key={val}
                  onClick={() => !answered && onSetTf(val)}
                  disabled={answered}
                  style={{
                    flex: 1, padding: '20px', borderRadius: 14, border: 'none',
                    cursor: answered ? 'default' : 'pointer', fontWeight: 700, fontSize: 18,
                    transition: 'all 0.2s',
        ...
        {(q.questionType === 'SHORT_ANSWER' || q.questionType === 'CODE_SNIPPET') && (
          <textarea
            ...
            disabled={answered}
            rows={4}
            placeholder={q.questionType === 'CODE_SNIPPET' ? 'Write your code here...' : 'Type your answer...'}
            style={{
              width: '100%', padding: '14px 16px', borderRadius: 12, border: '1px solid rgba(255,255,255,0.1)',
              background: 'rgba(0,0,0,0.3)', color: '#e2e8f0', fontSize: 15,
              fontFamily: q.questionType === 'CODE_SNIPPET' ? 'monospace' : 'inherit',
      ...
      {answered && (
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            ...
            {result!.xpEarned > 0 && (
              <div style={{ background: 'rgba(99,102,241,0.2)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: 10, padding: '6px 14px', color: '#a5b4fc', fontWeight: 700, fontSize: 15 }}>
                +{result!.xpEarned} XP
              </div>
            )}
          </div>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        ...
        {answered && (
          ...
          >
            {questionIndex + 1 >= 0 ? 'Next Question →' : 'View Summary'}
          </button>
        )}
      </div>
...
function SummaryScreen({ stats, concept, onRestart, onRepeat }: any) {
  ...
  const gradeColor = { S: '#f59e0b', A: '#10b981', B: '#6366f1', C: '#f97316', D: '#ef4444' }[grade];
    ...
    <div style={{ maxWidth: 600, margin: '0 auto', textAlign: 'center' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16, marginBottom: 32 }}>
        {[
          ...
          { label: 'Avg Time', value: `${(stats.avgTime / 1000).toFixed(1)}s`, color: '#f59e0b' },
          { label: 'Questions', value: stats.total, color: '#94a3b8' },
        ].map((s, i) => (
          <div
            key={i}
            style={{
